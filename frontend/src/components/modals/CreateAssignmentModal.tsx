...
export function CreateAssignmentModal({ courseId, onClose, onSuccess, token }: CreateAssignmentModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await fetch(`http://localhost:3001/api/courses/${courseId}/assignments`, {
        method: "POST",
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="glass-card w-full max-w-md p-6 relative">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-300 mb-1">Descripción / Instrucciones</label>
          ...
          <div>
            <label className="block text-sm font-medium text-neutral-300 mb-1">Fecha límite</label>
