...
import { Search, Download, CheckCircle, Clock, XCircle, FileImage } from "lucide-react";
import { exportToExcel } from "@/lib/exportUtils";
import { motion, AnimatePresence } from "framer-motion";

export function EvidenceTable({ data }: { data: Evidence[] }) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "APPROVED":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20"><CheckCircle size={14} /> Aprobado</span>;
      case "PENDING":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20"><Clock size={14} /> Pendiente</span>;
      case "REJECTED":
        return <span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20"><XCircle size={14} /> Rechazado</span>;
      default:
        return null;
    }
  };
    ...
    <div className="glass-card overflow-hidden flex flex-col">
      ...
      <div className="p-4 border-b border-white/10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-lg shadow-indigo-500/20"
        >
          <Download size={16} />
          Exportar Excel
        </button>
        ...
        <table className="w-full text-left text-sm text-neutral-300 min-w-[800px]">
          <thead className="text-xs uppercase bg-white/5 text-neutral-400 sticky top-0 backdrop-blur-xl">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold tracking-wider">Evidencia</th>
              <th className="px-6 py-4 font-semibold tracking-wider">Nombre Extraído</th>
            ...
            <AnimatePresence>
              {filteredData.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 font-medium text-white">{item.fullName}</td>
                  <td className="px-6 py-4">{item.activityType}</td>
                  <td className="px-6 py-4">
